...
public class Main {
    ...
    static final int terminalScore = 1000000;
    ...
    public static void train(Game.Mode gameMode, int iterations, int verboseFreq) {
        ...
        if (gameMode == Game.Mode.GENETIC) {
            ...
            while (geneticAlgorithm.maxScore < terminalScore && geneticAlgorithm.numGenerations <= iterations) {
                geneticAlgorithm.update();
                emulator.update();
                if (geneticAlgorithm.maxScore >= terminalScore
                        || geneticAlgorithm.maxScore >= Math.max(2, overallMaxScore * 2)) {
                    String folderName = String.format((new File(GENETIC_FILE_FORMAT)).getParent(),
                            geneticAlgorithm.maxScore, geneticAlgorithm.numGenerations);
                    ...
                    for (int i = 0; i < geneticAlgorithm.agents.size(); i++) {
                        ...
                        agent.brain.save(String.format(GENETIC_FILE_FORMAT, geneticAlgorithm.maxScore,
                                geneticAlgorithm.numGenerations, agent.player.score, i));
                        agent.brain.save(String.format(GENETIC_FILE_DEFAULT, agent.player.score, i));
                    }
                    overallMaxScore = geneticAlgorithm.maxScore;
                }

                if (geneticAlgorithm.numGenerations != prevGeneration && verboseFreq > 0
                        && geneticAlgorithm.numGenerations % verboseFreq == 0) {
                    System.out.print("[Generation " + geneticAlgorithm.numGenerations + "]: ");
                    System.out.println("max score = " + geneticAlgorithm.maxScore + ", max distance survived = "
                            + geneticAlgorithm.maxDistSurvived);
